...
function adjustPositionControlSidebarRight(sidebarClass) {
    const sidebarWidth = document.querySelector(sidebarClass + ".active")
        ? sidebarWidthRight
        : 0; // Adjust the width based on sidebar visibility

    // Set position control leaflet
    const controlContainersLeaflet = document.querySelectorAll(
    ...
    const controlContainers = document.querySelectorAll(
        ".container-control-basemap, .container-control-layer, .container-control-legend,.container-control-street-view, .control-legend-gee"
    );
...
function closeBasemapSidebar() {
    ...
    if (basemapSidebar.classList.contains("active")) {
        ...
        const basemapButton = document.querySelector(
            ".container-control-basemap button"
        );
...
function closeLayerSidebar() {
    ...
    if (layerSidebar.classList.contains("active")) {
        ...
        const layerButton = document.querySelector(
            ".container-control-layer button"
        );
...
function closeLegendSidebar() {
    ...
    if (layerSidebar.classList.contains("active")) {
        ...
        const layerButton = document.querySelector(
            ".container-control-legend button"
        );
        layerButton.classList.remove("active"); // Remove active class from the button
    }
}
...
const customControlBasemap = L.Control.extend({
    ...
    onAdd: function () {
        const container = L.DomUtil.create(
            "div",
            "btn btn-light btn-outline-secondary container-control-basemap"
        );
        ...
        container.addEventListener("click", function () {
            ...
            document
                .getElementById("sidebar-basemap")
...
const customControlLayer = L.Control.extend({
    ...
    onAdd: function () {
        const container = L.DomUtil.create(
            "div",
            "btn btn-light btn-outline-secondary container-control-layer"
        );
        const button = L.DomUtil.create(
            "button",
...
const customControlLegend = L.Control.extend({
    ...
    onAdd: function () {
        const container = L.DomUtil.create(
            "div",
            "btn btn-light btn-outline-secondary container-control-legend"
        );
        const button = L.DomUtil.create(
            "button",
...
const controlStreetView = L.Control.extend({
    ...
    onAdd: function () {
        const container = L.DomUtil.create(
            "div",
            "btn btn-light btn-outline-secondary container-control-street-view d-none"
        );
        const button = L.DomUtil.create(
            "button",
                ...
                const point = L.marker([lat, lon])
                    ...
                    .bindPopup(
                        '<a href="http://maps.google.com/maps?q=&layer=c&cbll=' +
                            ...
                            "," +
                            lon +
                            '&cbp=11,0,0,0" target="_blank"><b>View</b></a>'
                    )
                    .openPopup();
            });
        });
...
const legends = [
    {
        id: "#change_intensity, #igbp, #lst, #occurrence, #Transition, #water_season, #ChangeIntensity, #IGBP, #LST, #Occurrence, #WaterSeason, #Transition",
        url: "/legend_water.jpeg",
    },
...
const controlLegendGEE = L.Control.extend({
    ...
    onAdd: function () {
        const container = L.DomUtil.create(
            "div",
            "bg-light container-control-street-view d-none"
        );
...
function adjustPositionControlSidebarLeft(sidebarClass) {
    ...
    // Set position control leaflet
    const controlContainersLeaflet = document.querySelectorAll(
        ".leaflet-control-scale, .leaflet-control-geocoder, .leaflet-control-navbar, .leaflet-draw "
    );
    controlContainersLeaflet.forEach(function (controlContainerLeaflet) {
...
function closeAnalisisSidebar() {
    ...
    if (basemapSidebar.classList.contains("active")) {
        ...
        const basemapButton = document.querySelector(
            ".container-control-analisis button"
        );
        basemapButton.classList.remove("active"); // Remove active class from the button
    }
}
...
const customControlAnalisis = L.Control.extend({
    ...
    onAdd: function () {
        const container = L.DomUtil.create(
            "div",
            "btn btn-light btn-outline-secondary container-control-analisis"
        );
        const button = L.DomUtil.create(
            "button",
        ...
        container.addEventListener("click", function () {
            ...
            document
                .getElementById("sidebar-analisis")
                .classList.toggle("active");
            adjustPositionControlSidebarLeft(".sidebar-analisis.active"); // Call the function to adjust control positions
            const layerButton = document.querySelector(
                ".container-control-analisis button"
